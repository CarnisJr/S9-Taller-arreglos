...
// -----------------------------------------------------------------
    ...
    public String metodo1(){
    	...
    	int cantCarrosPlacaPB = contarCarrosQueComienzanConPlacaPB();
    	boolean hayCarrosMas24h = hayCarroCon24Horas();
    	String mensaje = "";
    	
    	if(hayCarrosMas24h == true) {
    		
    		mensaje = "Hay " + cantCarrosPlacaPB + " carros con una placa con prefijo PB." +
    				  " Hay carros parqueados por mas de 24 horas: si.";
    	}else {
    		
    		mensaje = "Hay " + cantCarrosPlacaPB + " carros con una placa con prefijo PB." +
    				  " Hay carros parqueados por mas de 24 horas: no.";
    	}
    ...
    public String metodo2(){
    	...
    	int cantCarros = desocuparParqueadero();
		String mensaje = "";

    	if(cantCarros > 0) {
    		
			mensaje = "Se han sacado " + cantCarros + " carros.";
    	}else {
    		
    		mensaje = "No se ha sacado ningun carro";
    	}
        return mensaje;
    }
    ...
    public double darTiempoPromedio() {
    	...
    	for(int i = 0; i < puestos.length; i++) {
    		...
    		if(puestos[i].estaOcupado()) {
				...
				tiempoPromedio += horaActual - puestos[i].darCarro().darHoraLlegada();
				cont++;
    		}
    	}
    	
    	tiempoPromedio = tiempoPromedio / cont;
    ...
    public Carro carroMasOchoHoras() {
    	...
    	Carro carroMasOchoHoras = null;
    	int mayorTiempoParqueado = 0;
    	int auxMayorTiempoParqueado = 0;

    	ArrayList<Carro> listaCarrosMasOchoHoras = new ArrayList<Carro>();
    	...
    	int i = 0;
		while(i < listaCarrosMasOchoHoras.size() - 1) {

			mayorTiempoParqueado = horaActual - listaCarrosMasOchoHoras.get(i).darHoraLlegada();
			auxMayorTiempoParqueado = horaActual - listaCarrosMasOchoHoras.get(i + 1).darHoraLlegada();
			
			if(mayorTiempoParqueado < auxMayorTiempoParqueado) {
				
				mayorTiempoParqueado = auxMayorTiempoParqueado;
				carroMasOchoHoras = listaCarrosMasOchoHoras.get(i + 1);
			}

			i++;
		}
    	
    	return carroMasOchoHoras;
    }
    ...
    public ArrayList<Carro> darCarrosMasDeTresHorasPaqueados(){
    ...
    public int desocuparParqueadero() {
    	...
    	return contCarrosSacados;
    }
    
    public int contarCarrosQueComienzanConPlacaPB(){
    	
    	int contCarrosPlacaPB = 0;
    	String auxPrefijoPlaca = "PB";
    	
    	for(int j = 0; j < puestos.length; j++) {
    		
    		if(puestos[j].estaOcupado()) {

    			if(puestos[j].darCarro().darPlaca().startsWith(auxPrefijoPlaca)) {
